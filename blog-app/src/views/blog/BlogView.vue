    ...
    <el-container class="me-view-container">
      ...
      <el-main>
        ...
        <div class="me-view-card">
          ...
          <div class="me-view-tag">
            投票统计：共有<span style="padding: 0 5px;color: red;">{{vote.total}}</span>人投票
          </div>

          <div class="me-view-tag">好评率：
            <el-progress type="circle" :percentage="vote.rate" :width="vote.width"></el-progress>
          </div>

          <div class="me-view-tag">
            你的投票：
            <el-rate v-model="vote.voteValue" v-bind:show-text="vote.showText" v-bind:texts="vote.texts" @change="score => publishVote(score)" style="display: inline-block;"></el-rate>
          </div>
...
<script>
  ...
  import {publishVote, getRate} from '@/api/vote'
  ...
  export default {
    ...
    methods: {
      ...
      publishVote(score) {
        let that = this;
        that.vote.articleId = that.$route.params.id;
        that.vote.voteValue = score;
        publishVote(that.vote).then(data => {
          if (score > 0) {
            that.$message({type: 'success', message: '投票成功', showClose: true});
          }
          that.vote.voteValue = data.data.value;
          that.getRate();
        }).catch(error => {
          if (error !== 'error') {
            that.$message({type: 'error', message: '投票失败', showClose: true});
          }
        });
      },
      getRate() {
        let that = this;
        getRate(that.$route.params.id).then(data => {
          that.vote.rate = data.data.rate;
          that.vote.total = data.data.total;
        }).catch(error => {
          if (error !== 'error') {
            that.$message({type: 'error', message: '统计投票失败', showClose: true});
          }
        });
      }
    },

...
<script>
  ...
  import {listArchivesMy} from '@/api/article'
  ...
  export default {
    ...
    methods: {
      ...
      changeArchive(year, month) {
        ...
        this.$router.push({path: `/mywenzhang/${userid}/${year}/${month}`})
      },
        listArchivesMy(userid) {
            listArchivesMy(userid).then((data => {
          this.archives = data.data
        })).catch(error => {
          that.$message({type: 'error', message: '我的文章载失败!', showClose: true})
        })
      }
    }
  }
